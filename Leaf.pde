class Leaf
{
  ...
  void Update()
  {
    if (Free)
    {
      PVector acc = PVector.add(g_LeafGravity, PVector.mult(new PVector(noise(frameCount/(int)random(1, 3)), noise(frameCount/(int)random(2, 5))), g_NoiseMaxStrengthMultiplier));
      Vel.add(acc);
