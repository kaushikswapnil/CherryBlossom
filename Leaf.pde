class Leaf
{
  ...
  void Update()
  {
    if (Free)
    {
      PVector acc = PVector.add(g_LeafGravity, PVector.mult(PVector.fromAngle(map(noise(Pos.x, Pos.y, frameCount/100), 0.0, 1.0, 0.0, TWO_PI)), g_NoiseMaxStrengthMultiplier));
      Vel.add(acc);
